...
from ankabut.scripts.download_progress import DownloadProgress
from multiprocessing import Process, Queue, freeze_support
from ankabut.gui.mainwindow_ui import Ui_MainWindow
from ankabut.scripts import initialization as init
from ankabut.scripts.ask_download import AskDialog
from ankabut.scripts.search_engine import Google
from PyQt5.QtGui import QMovie, QIcon, QPixmap
from ankabut.scripts.settings import Settings
from ankabut.scripts.download import Download
from ankabut.scripts.add_link import AddLink
from ankabut.scripts.scraper import Scraper
from ankabut.scripts import ankabut_config
from ankabut.scripts import exceptions
from urllib.parse import unquote
from ankabut.scripts import resources
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def delete_button_clicked(self):
        ...
        ans=QtWidgets.QMessageBox.question(self,'Ankabut','are you sure about removing the this download ?')
    ...
    def resume_button_clicked(self):
        ...
        if self.downloadResume_pb.status == 'complete':
            ans=QtWidgets.QMessageBox.question(self,'Ankabut', 'would like to restart the download ? ')
            if ans == QtWidgets.QMessageBox.Yes:
                ...
                try:
                    ...
                    QtWidgets.QMessageBox.warning(self,'Ankabut',str(e))
    ...
    def thread_creator(self):
        exts = ankabut_config.get_exts(self.category.lower())
        if not self.links:
    ...
    def search_button_clicked(self):
        try:
            ...
            if search_t == 0: 
                ...
                query = ankabut_config.get_query(language,self.category.lower()) 
                self.main_query =file_name+' '+query 

            # google advanced search query
            elif search_t == 1:
                types= ankabut_config.get_exts(self.category.lower())
                types = '|'.join(types)
                ad_query = ankabut_config.get_advanced_query()
    ...
    def init_lang_category(self):
        cats=ankabut_config.get_exts()
        langs=ankabut_config.get_langs()
    ...
    def init_properties(self):
        ...
        init.init()
        ankabut_config.config_check()
        

        self.setWindowTitle('Ankabut')
        self.setWindowIcon(QIcon(':/icons/ankabut.ico'))
